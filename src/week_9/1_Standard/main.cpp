...
int main(int argc, char **argv)
{
    ...
    deltaX2 = previousField.dx * previousField.dx;
    deltaY2 = previousField.dy * previousField.dy;

    double a = 0.5;
    timeStep = (deltaX2 * deltaY2) / (2 * a * (deltaX2 + deltaY2));
    ...
    for (iteration = initialIteration; iteration < initialIteration + numTimeSteps; iteration++) {
        ...
        update_boundary_temperature( &currentField, &previousField, a, timeStep);
        start_halo_exchange( &previousField, &parallelInfo);
        update_interior_temperature( &currentField, &previousField, a, timeStep);
