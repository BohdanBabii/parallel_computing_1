...
void update_interior_temperature(Field *curr, Field *prev, double a, double dt) {
    ...
    for (i = 1; i < curr->nx + 1; i++) {
        for (j = 1; j < curr->ny + 1; j++) {
            ...
            curr->data[ic] = prev->data[ic] + ((a * dt) / (dx_2)) * (prev->data[ir] - 2 * prev->data[ic] + prev->data[il]) + ((a * dt) / (dy_2)) * (prev->data[iu] - 2 * prev->data[ic] + prev->data[id]);
        }
    }
}
...
void update_boundary_temperature(Field *curr, Field *prev, double a, double dt) {
    ...
    for (j = 1; j < curr->ny + 1; j++) {
        ...
        curr->data[ic] = prev->data[ic] + ((a * dt) / (dx2)) * (prev->data[ir] - 2 * prev->data[ic] + prev->data[il]) + ((a * dt) / (dy2)) * (prev->data[iu] - 2 * prev->data[ic] + prev->data[id]);
    }
